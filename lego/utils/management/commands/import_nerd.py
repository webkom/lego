...
class Command(BaseCommand):
    ...
    def load_initial_fixtures(self):
        ...
        self.load_fixtures(
            ["files/fixtures/initial_files.yaml", "tags/fixtures/initial_tags.yaml"]
        )
    ...
    def run(self, *args, **options):
        ...
        if os.path.isfile(f"{IMPORT_DIRECTORY}/1_nerd_export_group_objects.yaml"):
            ...
            nerd_groups = self.load_yaml(
                f"{IMPORT_DIRECTORY}/1_nerd_export_group_objects.yaml"
            )
            for group_model in nerd_groups:
                ...
                print(
                    f"[SUCCESS] Added {group_name} to the import tree\n------------------"
                )
