...
from lego.apps.search import signals


class SignalTestCase(TestCase):

    @override_settings(TESTING=False)
    @mock.patch('lego.apps.search.signal_handler.AsyncSignalHandler.on_save')
    def test_handler_save(self, mock_on_save):
        ...
        mock_on_save.assert_called_once_with(instance_mock)

    @override_settings(TESTING=False)
    @mock.patch('lego.apps.search.signal_handler.AsyncSignalHandler.on_delete')
