...
def track_instance(instance):
    ...
    if isinstance(instance, User):
        ...
        if pool:
            ...
            properties.update(
                {"pool_capacity": 0, "pool_spots_left": 0, "pool_id": None}
            )
        ...
        if earliest_registration and instance.registration_date:
            properties["seconds_after_opening"] = int(
                (instance.registration_date - earliest_registration).total_seconds()
            )

        if event.start_time and instance.unregistration_date:
            properties["seconds_before_start"] = int(
                (event.start_time - instance.unregistration_date).total_seconds()
            )
        ...
        track(
            instance.follower, "follower.follow", {"target_event": instance.target_id}
        )
    elif isinstance(instance, FollowCompany):
        # Track user following  company
        track(
            instance.follower, "follower.follow", {"target_company": instance.target_id}
        )
