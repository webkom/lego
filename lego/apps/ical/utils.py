...
def add_meeting_to_ical_feed(feed, meeting):
    desc_context = {
        ...
        'reportAuthor': meeting.report_author.full_name if meeting.report_author else 'Ikke valgt',
        'url': meeting.get_absolute_url()
    }
    desc_template = loader.get_template('ical/meeting_description.txt')
    feed.add_item(
        title=meeting.title, unique_id=f'meeting-{meeting.id}@abakus.no',
        link=meeting.get_absolute_url(), description=desc_template.render(desc_context),
        start_datetime=meeting.start_time, end_datetime=meeting.end_time, location=meeting.location
    )
