...
_test_payload_error_messages = {
    "required_field": "This field is required.",
    "not_blank_field": "This field may not be blank.",
    "not_null_field": "This field may not be null.",
    "valid_choice": "is not valid choice.",
    "wrong_format": "Datetime has wrong format.",
}
...
class ListEventsTestCase(BaseAPITestCase):
    ...
    def test_with_webkom_user(self):
        ...
        event_response = self.client.get(_get_list_url())
        self.assertEqual(event_response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(event_response.json()["results"]), 13)
...
class CreateEventsTestCase(BaseAPITestCase):
    ...
    def test_fields_required_on_create(self) -> None:
        ...
        for data in test_data:
            with self.subTest():
                ...
                for field in _date_fields:
                    self.assertIn(
                        _test_payload_error_messages["wrong_format"],
                        response.data.get(field)[0],
                    )
                self.assertEqual(
...
class AllergiesTestCase(BaseAPITestCase):
    ...
    def setUp(self):
        self.abakus_user = User.objects.get(pk=1)
        self.allergies_user = User.objects.get(pk=11)
        self.event = Event.objects.get(title="ALLERGIES_EVENT")
