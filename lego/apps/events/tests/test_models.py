...
from lego.apps.events.exceptions import NoAvailablePools, UserNotAdmin
...
class AdminRegistrationTestCase(TestCase):
    ...
    def test_admin_registration(self):
        event = Event.objects.get(title='POOLS_NO_REGISTRATIONS')
        request_user, user = get_dummy_users(2)
        ...
        event.admin_register(request_user, user, pool)
    ...
    def test_ar_with_wrong_pool(self):
        ...
        request_user, user = get_dummy_users(2)
        event_one.created_by = request_user
        ...
        with self.assertRaises(ValueError):
            event_one.admin_register(request_user, user, wrong_pool)
    ...
    def test_ar_without_permissions_for_admin(self):
        event = Event.objects.get(title='POOLS_NO_REGISTRATIONS')
        request_user, user = get_dummy_users(2)
        pool = event.pools.first()
        ...
        with self.assertRaises(UserNotAdmin):
            event.admin_register(request_user, user, pool)
        self.assertEqual(event.number_of_registrations, e1_no_of_regs_before)
        self.assertEqual(pool.registrations.count(), pool_no_of_regs_before)
