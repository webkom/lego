...
class LDAPSystem(ExternalSystem):
    ...
    def filter_groups(self, queryset):
        ...
        return queryset.filter(
            Q(type=GROUP_COMMITTEE) | Q(name__in=settings.LDAP_GROUPS)
        )
    ...
    def add_group(self, group):
        members = list(
            group.memberships.distinct("user").values_list("user__username", flat=True)
        )
    ...
    def delete_excess_groups(self, groups):
        allowed_groups = [
            name.lower() for name in groups.values_list("name", flat=True)
        ]
