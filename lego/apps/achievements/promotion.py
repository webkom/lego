...
def check_leveled_promotions(user: User, identifier: str, input_achievements: dict):
    ...
    while True:
        ...
        if next_achievement_data["requirement_function"](user):
            ...
            current_achievement.rarity = next_achievement_data["rarity"]
    ...
    if owner == user and meeting.invited_users.count() == 1:
        if not Achievement.objects.filter(
            ...
            Achievement.objects.create(
                ...
                user=user,
                rarity=MEETING_ACHIEVEMENTS["meeting_hidden"]["rarity"],
