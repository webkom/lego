...
class ListCompanyInterestTestCase(BaseAPITestCase):
    fixtures = ['test_abakus_groups.yaml', 'test_company_interest.yaml', 'test_users.yaml']
...
class CreateCompanyInterestTestCase(BaseAPITestCase):
    ...
    def test_create_with_any_user(self):
        response = self.client.post(_get_company_interests(), _test_company_interest_data[0])
