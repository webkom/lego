...
class CompanyInterestHandler(Handler):
    ...
    def handle_create(self, instance, **kwargs):
        ...
        if (booking_period_from <= (current_date.month, current_date.day) <= booking_period_to):
            ...
            if (mail_context["readme"]): recipients.append(f"lederreadme@{settings.GSUITE_DOMAIN}") 
            send_email.delay(
                to_email= recipients,
